...
function M.show()
  ...
  canvas = hs.canvas.new({x=geometry.x, y=geometry.y, w=width, h=height})
    :level(hs.canvas.windowLevels.floating):clickActivating(false)
  local function append(element) canvas:appendElements(element); return canvas:elementCount() end
  local function text(value, x, y, w, h, size, color)
    append({ type = 'text', text = value, textSize = size, textFont = '.AppleSystemUIFont',
      textColor = { hex = color }, frame = { x = x, y = y, w = w, h = h } })
  end
  append({ type = 'rectangle', action = 'fill', fillColor = { hex = '#141b28' },
    roundedRectRadii = { xRadius = 20, yRadius = 20 }, frame = { x = 0, y = 0, w = width, h = height } })
  ...
  for i = geometry.first, geometry.last do
    ...
    backgrounds[i] = append({ type = 'rectangle', action = 'fill', fillColor = { hex = '#222e42' },
      roundedRectRadii = { xRadius = 8, yRadius = 8 }, frame = { x = x, y = y, w = tileW, h = tileH } })
    ...
    append({ type = 'rectangle', id = 'tile' .. i, action = 'fill', fillColor = { alpha = 0 },
  ...
  for _, nav in ipairs({{'previous', '‹', width - 123, page > 1},
                        {'next', '›', width - 40, page < geometry.pages}}) do
    text(nav[2], nav[3] + 8, height - 35, 24, 30, 24, nav[4] and '#eef2fa' or '#4a5364')
    if nav[4] then
      append({type='rectangle', id=nav[1], action='fill', fillColor={alpha=0},
        frame={x=nav[3],y=height-38,w=36,h=36},trackMouseUp=true,trackMouseByBounds=true})
    end
  end
  canvas:mouseCallback(function(_, event, id)
    if id == 'close' and event == 'mouseUp' then M.hide(); return end
    if event == 'mouseUp' and (id == 'next' or id == 'previous') then
      page = page + (id == 'next' and 1 or -1); M.show(); return
    end
    if event == 'mouseUp' and id == 'mode' then
      mode = mode == 'replace' and 'copy' or 'replace'; M.show(); return
    end
    local index = type(id) == 'string' and tonumber(id:match('^tile(%d+)$'))
    if not index or not tiles[index] or not backgrounds[index] then return end
    if event == 'mouseEnter' then canvas:elementAttribute(status, 'text', tiles[index].title) end
    if event == 'mouseEnter' or event == 'mouseExit' then
      canvas:elementAttribute(backgrounds[index], 'fillColor', { hex = event == 'mouseEnter' and '#344866' or '#222e42' })
...
function M.toggle()
  if canvas and canvas:isShowing() then M.hide() else page = 1; M.show() end
end

function M.bindHotkeys(modifiers, key)
  if toggleKey then toggleKey:delete() end
...
function M.stop()
  ...
  if toggleKey then toggleKey:delete(); toggleKey=nil end
