...
function M.calculate(screen, count, requestedPage, maxColumns)
  ...
  return {x=screen.x + (screen.w-width)/2, y=screen.y, w=width, h=height,
    columns=columns, rows=rows, capacity=capacity, pages=pages, page=page,
    first=(page-1)*capacity+1, last=math.min(count,page*capacity),
    tileWidth=tileWidth,tileHeight=tileHeight,padding=padding,gap=gap}
end
return M
