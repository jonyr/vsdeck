...
function M.urls(urls)
  if type(urls) ~= 'table' then return nil, t('web.url_list') end
  local count = 0
  ...
  if count == 0 then return nil, t('web.url_empty') end
  local copy = {}
...
function M.request(shortcut, registry)
  if type(shortcut) ~= 'table' then return nil, nil, t('web.config_missing') end
  local browser = shortcut.browser or 'safari'
  local spec = type(browser) == 'string' and registry[browser]
  if not spec then return nil, nil, t('web.invalid_browser') end
