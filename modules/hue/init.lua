...
function M.run(config, light)
  ...
  local function request(method, url, body, callback)
    hs.http.doAsyncRequest(url, method, body, {['Content-Type']='application/json'}, function(status, raw)
      if pending[id] ~= entry then return end
      if status ~= 200 then finish(t('hue.contact')); return end
      local ok, data = pcall(hs.json.decode, raw)
      if not ok or type(data) ~= 'table' then finish(t('hue.response')); return end
      for _, item in ipairs(data) do
        if item.error then finish(t('hue.rejected')); return end
      end
      callback(data)
    end)
  end
  entry.timer = hs.timer.doAfter(20, function() finish(t('hue.timeout')); end)
  entry.task = hs.task.new('/usr/bin/security', function(code, stdout)
    if pending[id] ~= entry then return end
    local key = (stdout or ''):gsub('%s+$', '')
    if code ~= 0 or not key:match('^[%w%-]+$') then
      finish(t('hue.key_read')); return
    end
    local url = 'http://' .. config.bridge .. '/api/' .. key .. '/lights/' .. tostring(light.lightId)
    request('GET', url, nil, function(data)
      if not data.state or type(data.state.on) ~= 'boolean' then finish(t('hue.unknown')); return end
      if data.state.reachable == false then finish(t('hue.unreachable')); return end
      ...
      request('PUT', url .. '/state', hs.json.encode({on=desired}), function(result)
        ...
        for _, item in ipairs(result) do
          if item.success and item.success[expected] == desired then
            finish(t(desired and 'hue.on' or 'hue.off', { name = light.title or t('hue.light') }), 'success'); return
          end
        end
  ...
  if not entry.task or not entry.task:start() then finish(t('hue.key_access')) end
end
return M
