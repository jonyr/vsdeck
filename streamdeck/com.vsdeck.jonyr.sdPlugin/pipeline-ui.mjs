import {fields, pipelineSettings} from './pipeline-settings.mjs';
...
$('save').addEventListener('click', () => {
  ...
  try {
    pending = {...settings,...pipelineSettings(Object.fromEntries(fields.map(key => [key,$(key).value])))};
...
window.connectElgatoStreamDeckSocket = async (port,uuid,registerEvent,info,actionInfo) => {
  ...
  strings=(await (await fetch('pipeline-locales.json')).json())[locale];
