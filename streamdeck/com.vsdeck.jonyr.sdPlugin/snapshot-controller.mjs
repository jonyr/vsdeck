...
import {snapshotSettings, snapshotKey} from './snapshot-settings.mjs';

// Poll independently per destination. Never retry a create after an uncertain response.
export class SnapshotController {
  ...
  appear(context, raw) {
    try {
      const settings = snapshotSettings(raw), key = snapshotKey(settings);
  ...
  image(context, state, color, foreground) {
    const image = awsImage('snapshot',state,color,foreground);
  ...
  async press(id, context, raw) {
    let settings;
    try { settings = snapshotSettings(raw); } catch { this.image(context,'error'); this.send('showAlert',context); return; }
