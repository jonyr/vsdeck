--- Validate adapter selection and argument construction without launching browsers.
-- @script tests.browser_test
...
hs = {
  application = {
    pathForBundleID = function(id)
      return '/Applications/' .. id .. '.app'
    end,
  },
  fs = {
    attributes = function()
      return exists and 'directory' or nil
    end,
  },
  task = {
    new = function(executable, callback, args)
...
package.loaded['modules.deck.browsers.safari'] = {
  open = function(request, spec)
...
assert(#launches == n)
print(
  'PASS: Chrome explicit profile, Brave default, Safari explicit/default, URL order, invalid browser/profile/URL, missing profile.'
)
