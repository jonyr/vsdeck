--- Simulate Bridge/Keychain responses and reject duplicate or unconfirmed light operations.
-- @script tests.hue_test
...
local alerts, puts, callback, state, reply
...
hs = {
  ...
  http = {
    doAsyncRequest = function(_, method, body, _, cb)
      if method == 'GET' then
        cb(200, state)
      else
        puts = puts + 1
        assert(body.on == false)
        cb(200, reply)
      end
    end,
  },
...
reset()
local m = require('modules.hue')
m.run(config, light)
m.run(config, light)
assert(alerts[1]:find('en curso'))
callback(0, 'testkey\n')
assert(puts == 1)
assert(alerts[2] == 'VSDeck\nLuz: apagada')
reset()
m = require('modules.hue')
m.run(config, light)
callback(1, '')
assert(puts == 0)
assert(alerts[1]:find('llavero'))
reset()
...
callback(0, 'testkey')
assert(puts == 0)
assert(alerts[1]:find('accesible'))
reset()
state = { { error = { type = 1 } } }
m = require('modules.hue')
m.run(config, light)
callback(0, 'testkey')
assert(puts == 0)
assert(alerts[1]:find('rechazó'))
reset()
reply = {}
m = require('modules.hue')
m.run(config, light)
callback(0, 'testkey')
assert(alerts[1]:find('no confirmó'))
reset()
m = require('modules.hue')
m.run({ bridge = 'invalid' }, light)
assert(callback == nil)
