...
local function parameters(text)
  local names = {}
  for name in text:gmatch('{([%w_]+)}') do names[#names + 1] = name end
  table.sort(names)
...
for key in pairs(en) do assert(es[key] ~= nil, 'Extra English key: ' .. key) end
...
local saved = en['ai_text.copied']; en['ai_text.copied'] = nil
assert(i18n.t('ai_text.copied') == es['ai_text.copied']); en['ai_text.copied'] = saved
...
i18n.setLanguage('unsupported'); assert(i18n.language() == 'es')
for key in pairs(i18n.catalog('deck.')) do assert(key:sub(1,5) == 'deck.') end
...
i18n.setLanguage('en'); package.loaded['modules.ai_text.actions'] = nil
...
print('PASS: ' .. count .. ' bilingual keys, parameters, plural forms, fallback, locale normalization and stable action contracts.')
