--- Verify notification channels, severity preferences, appearance and completion defaults.
-- @script tests.notification_routing_test
...
n = load({ backend = 'both' })
n.info('ai_text.processing')
assert(#native == 1 and #alerts == 1)
n = load({ backend = 'notify', levels = { info = { enabled = false }, error = { backend = 'alert', duration = 9 } } })
n.info('ai_text.processing')
assert(#native == 0 and #alerts == 0)
n.error('ai_text.empty_response')
assert(#native == 0 and #alerts == 1 and alerts[1].duration == 9)
...
n.text('success', 'Done', { final = true })
assert(#native == 0 and #alerts == 1)
n = load({ backend = 'alert', finalBackend = 'notify' })
n.text('success', 'Done', { final = true })
assert(#native == 1 and #alerts == 0)
n = load({ backend = 'invalid', levels = { error = true }, duration = -1 })
n.error('ai_text.empty_response')
assert(#native == 1 and #alerts == 0)
print(
  'PASS: all channels, translation, level overrides, mute, custom appearance, task completion and invalid settings.'
)
