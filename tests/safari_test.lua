--- Simulate native menu selection and guarded tab opening without real keystrokes.
-- @script tests.safari_test
...
local app = {
  ...
  focusedWindow = function()
    return active or old
  end,
  findMenuItem = function(_, path)
    return { enabled = path[3] == 'New Work Window' }
  end,
...
hs = {
  ...
  timer = {
    doEvery = function(_, fn)
      return schedule(true, fn)
    end,
    doAfter = function(_, fn)
      return schedule(false, fn)
    end,
  },
  eventtap = {
...
local function drain()
  ...
  while #queue > 0 do
    steps = steps + 1
    assert(steps < 200, 'sequence did not terminate')
    local item = table.remove(queue, 1)
    if not item.timer.stopped then
      item.fn()
      if item.repeating and not item.timer.stopped then
        queue[#queue + 1] = item
      end
    end
  end
...
assert(
  table.concat(events, '|')
    == 'l|https://example.com|return|t|l|https://example.org|return|t|l|https://example.net|return'
)
events = {}
active = nil
assert(safari.openProfile('Work', { 'https://example.com' }))
drain()
...
local captured
package.loaded['modules.deck.browsers.process'] = {
  launch = function(executable, args)
...
assert(require('modules.deck.browser').open({ browser = 'safari', profile = 'Work', urls = { 'https://example.com' } }))
drain()
